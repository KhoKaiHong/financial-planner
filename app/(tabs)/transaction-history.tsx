  ...
  const generateCostCuttingTips = (
    ...
    Object.entries(categoryTotals).forEach(([category, total]) => {
      ...
      if (budget && total > budget * 1.1) {
        tips.push(
          `🚨 Spending in "${category}" exceeded your budget of RM ${budget.toFixed(
            2
          )}.`
        );
      } else if (budget && total > budget * 0.8) {
        tips.push(
          `⚠️ You're nearing your budget in "${category}". Consider reviewing your spending.`
        );
      } else if (!budget && total > totalSpending * 0.3) {
        tips.push(
          `💡 You're spending a lot in "${category}" (RM ${total.toFixed(
            2
    ...
    if (Object.keys(categoryTotals).length <= 1 && totalSpending > 50) {
      tips.push(
        "📊 Most of your spending is concentrated in one category. Diversify to balance your expenses."
      );
    }
    ...
    if (smallTransactionCount > 10) {
      tips.push(
        "🔍 You have many small transactions. Review if some can be avoided to save money."
      );
    }
    ...
    if (totalSpending === 0) {
      tips.push(
        "✅ No spending detected this month. Great job controlling your expenses!"
      );
    }
        ...
        >
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent className="bg-background border border-border rounded-md">
              {["Food", "Entertainment", "Transport", "Others", "Bills"].map(
                (category) => (
                  <DropdownMenuItem
                    key={category}
      ...
      <Modal visible={showModal} transparent animationType="slide">
        <View className="flex-1 justify-center items-center bg-black bg-opacity-50">
          <View className="bg-white dark:bg-zinc-900 p-6 rounded-lg w-80">
            ...
            {["Food", "Entertainment", "Transportation", "Bills", "Others"].map(
              (category) => (
                <TouchableOpacity
                  key={category}
